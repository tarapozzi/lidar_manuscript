...
id.gender.male <- round((35/58)*100, digits=0)
id.gender.female <- round((20/58)*100, digits=0)
...
or.years <- round(summarise(oregon, avg=mean(years)), digits=1) ### THIS ISNT WORKING
...
survey.responses <- join(survey.responses, comm.subset[, c("Email", "comm_total")], by= "Email", type="left")
...
survey.responses <- join(survey.responses, expertise.subset[, c("Email", "expert_total")], by= "Email", type="left")
...
or.lidaruse <- round((36/58)*100, digits=2)

count(washington$lidaruse==1)
wa.lidaruse <- round((35/54)*100, digits=2)
...
full.mod <- stan_glmer(lidaruse ~ incr_sev_flood + experience_1 + prepared +
                           + soep_1 + future_1 + gov_trust + science_trust + flood_zone + prop_lidar_network + comm_total + expert_total
                           + (1|location),
...
#summary.stats <- matrix(c(-4.2, 2.1, -7.7, -0.8, 0.5, 0.4, -0.2, 1.2, 0.2, 0.7, -1.0, 1.3, 0.5, 0.4, -.1, 1.1, 0.1, 0.1, -0.5, 2.4, 0.9, 0.9, -0.1, 0.3, -0.4, 0.4, x, x, 0.0, 0.4, x, x, 0.9, 0.5, x, x, 5.5, 1.0, x, x, 0.0, 0.0, x, x, 0.0, 0.0, x, x.....), ncol=4, byrow=TRUE) #this is the plogis values for mean and SD ### UPDATE THESE NUMBERS
...
Social learning plays an important role in technology adoption... our results support that this is the most influential factor of lidar adoption.
In Idaho, there is a slope of `r b.id`, 'r b.or' for Oregon, and `r b.wa` for Washington. Awareness was measured by asking the survey respondent about the perception of the average severity of flood damage in their community decreasing, staying the same, or increasing. Overall, there was a positive correlation between awareness of future risk and lidar use. Furthermore, Washington had a stronger correlation. This could be due to Washington having seen signifcantly more damage and portion of population at risk to floods shown in $\label{tab:state_comp}$. 


```{r, proportion of lidar users, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="\\label:awarnessplot"}
